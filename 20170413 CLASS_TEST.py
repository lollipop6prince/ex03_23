...
X_train, X_test, y_train, y_test = train_test_split(iris_X, iris_y, test_size=0.50)
## 將iris_X和iris_y這2個數據集分成訓練集和測試集，其中測試集佔數據集的50%
...
## 數據標準化的範例
a = np.array([[9, 2.8, 7],[-200, 8, -3],[520, 1000, 30]], dtype=np.float64)
...
## 數據標準化對機器學習成效的影響
# 建立具有2種屬性(n_features=2)的200筆數據(n_samples=200)，其中有2個相關的屬性(n_informative=2)，
# random_state是隨機數的種子，n_clusters_per_class每個分類的集群數
X, y = make_classification(n_samples=200, n_features=2,
                           n_redundant=0, n_informative=2,
                           random_state=50, n_clusters_per_class=1,
                           scale=100)
